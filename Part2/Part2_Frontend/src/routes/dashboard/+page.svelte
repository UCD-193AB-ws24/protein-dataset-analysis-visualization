<script lang="ts">
  ...
  import { fetchUserFileGroups, deleteGroup } from './services/services';
  ...
  async function fetchGroups() {
    ...
    try {
      const data = await fetchUserFileGroups(access_token, id_token);
  ...
  async function handleDeleteGroup(groupId: string) {
    ...
    try {
      await deleteGroup(groupId, access_token);
      userFileGroups = userFileGroups.filter((g) => g.id !== groupId);
      filteredFileGroups = filteredFileGroups.filter((g) => g.id !== groupId);
    } catch (err: unknown) {
      errorMessage =
  ...
  $: {
    ...
    filteredFileGroups = userFileGroups.filter((group) => {
      ...
      const matchesSearch =
        ...
        group.genomes.some((genome) => genome.toLowerCase().includes(q));
...
<div class="w-[95%] max-w-[1600px] mx-auto py-8">
  ...
  {#if filteredFileGroups.length > 0}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each filteredFileGroups as group (group.id)}
        <ProjectCard
          group={group}
