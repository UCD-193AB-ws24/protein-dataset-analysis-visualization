<script lang="ts">
	...
	async function saveSharedGroup() {
		...
		if (!title || numGenes == null || numDomains == null || !selectedGraph?.genomes?.length) {
	...
	function filterGraph() {
		...
		filteredGraph.links = selectedGraph.links.filter(
			(link) =>
	...
	<div class="flex gap-6">
		...
		<div class={`transition-all duration-300 ${isPanelCollapsed ? 'w-12' : 'w-80'} shrink-0`}>
			<div class="sticky top-[5.5rem]">
				<div
					class="bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden flex flex-col max-h-[calc(100vh-7rem)]"
				>
					...
					<div class="p-2 bg-white border-b border-slate-200 flex justify-end shrink-0">
						...
						>
							<svg
								...
								class={`transition-transform ${isPanelCollapsed ? 'rotate-180' : ''}`}
							>
									...
									{#if selectedGraph.genomes}
										<div class="space-y-2">
											...
											<div class="bg-green-50 rounded-lg p-2.5">
												<h4 class="text-xs font-medium text-green-800 mb-1.5">
													Selected Genomes (drag to reorder)
												</h4>
		...
		<div class="flex-1 min-w-0">
			...
			{:else}
				...
				{#if ((isAuthenticated && graphs.length > 0) || (groupId && (matrixFiles.length > 0 || coordinateFile))) && (!groupUserId || user?.profile?.sub == groupUserId)}
					<div class="mb-8">
										...
										{#if coordinateFile}
											<div>
												...
												<div class="flex flex-col gap-1.5">
													<a
														href={coordinateFile.url}
														target="_blank"
														rel="noopener noreferrer"
														class="inline-block max-w-full"
													>
														<button
															class="inline-flex w-full items-center px-3 py-1.5 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors duration-200 cursor-pointer"
														>
															<span class="truncate flex-1 text-left"
																>{coordinateFile.original_name}</span
															>
															<svg
																xmlns="http://www.w3.org/2000/svg"
																width="16"
										...
										{#if matrixFiles.length > 0}
											<div>
												<h4 class="text-lg font-medium text-slate-700 mb-2">
													Matrix File{matrixFiles.length > 1 ? 's' : ''}
												</h4>
												<div class="flex flex-col gap-1.5">
				...
				{#if isAuthenticated && groupUserId && user?.profile?.sub !== groupUserId}
					<div class="p-6 bg-white rounded-lg shadow-sm border border-slate-200">
						...
						<p class="text-slate-600 mb-4">
							Add group to your account for easy access and tracking.
						</p>
						<button
					...
					{:else if !graphs.length}
						<div class="flex flex-col items-center justify-center py-8">
							...
							<p class="text-slate-600 text-center">
								Upload your files to visualize protein relationships
							</p>
						</div>
					{:else if !filteredGraph.nodes.length}
						<div class="flex justify-center items-center py-8">
							<p class="text-slate-600 text-center">
								Select genomes from the left panel to view the graph
							</p>
						</div>
					{:else}
						<Chart
